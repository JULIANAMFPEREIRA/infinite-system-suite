...
export default function RevenueExpensesChart({ receber, pagar }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-xl border border-border p-5 shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 relative z-10">
          ...
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${
            ...
            <div className="text-right">
              <p className="text-[10px] uppercase tracking-wider opacity-70">Lucro Líquido</p>
              <p className="text-sm font-bold">{fmt(lucroMesAtual)}</p>
            </div>
          </div>
        </div>

        {/* Chart */}
        <div className="h-[220px] sm:h-[280px] relative z-10">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
              ...
              <Area
                ...
                dot={false}
                activeDot={{ r: 4, stroke: "hsl(152, 69%, 50%)", strokeWidth: 2, fill: "hsl(222, 18%, 12%)" }}
              />
              <Area
                type="monotone"
                dataKey="despesa"
                ...
                dot={false}
                activeDot={{ r: 4, stroke: "hsl(0, 72%, 55%)", strokeWidth: 2, fill: "hsl(222, 18%, 12%)" }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="flex items-center justify-center gap-6 mt-3 relative z-10">
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-[2px] rounded bg-[hsl(152,69%,50%)]" />
            <span className="text-[10px] text-muted-foreground">Receitas</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-[2px] rounded bg-[hsl(0,72%,55%)]" />
      ...
      {/* Smart Alert */}
      {isLoss && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[hsl(0,72%,55%)]/10 border border-[hsl(0,72%,55%)]/20 animate-fade-in">
          <AlertTriangle size={16} className="text-[hsl(0,72%,55%)] shrink-0" />
          <p className="text-[11px] text-[hsl(0,72%,55%)] font-medium">
            ALERTA: Despesas maiores que receitas neste mês — Prejuízo de {fmt(Math.abs(lucroMesAtual))}
          </p>
        </div>
      )}
    </div>
