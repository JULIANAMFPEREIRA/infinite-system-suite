...
const FinanceiroPagar = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      await createConta.mutateAsync({ descricao: desc, valor, data_vencimento: vencimento || null, status: "pendente" });
      toast.success("Conta adicionada"); setDesc(""); setValor(0); setVencimento(""); setShowForm(false);
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {showForm && (
        <div className="bg-card border border-border rounded p-3 flex items-end gap-3">
            ...
            <tbody>
              {contas?.map(c => (
                <tr key={c.id} className="border-b border-border last:border-b-0 hover:bg-secondary/30">
                  ...
                  <td className="px-2.5 py-1.5 text-center">
                    {c.status === "pendente" && (
                      <button onClick={() => handlePagar(c.id)} className="p-1 rounded hover:bg-success/15 text-muted-foreground hover:text-success transition">
                        <Check size={13} />
                      </button>
                    )}
                  </td>
